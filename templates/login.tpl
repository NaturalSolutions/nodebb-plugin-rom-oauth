...
<div id="row-btn-portal" class="row">
    <div class="col-md-12">
      <!-- BEGIN authentication -->
        <!-- IF @first -->
        <a class="btn btn-primary" rel="nofollow noopener noreferrer" target="_top" href="{config.relative_path}{authentication.url}">Se connecter</a>
        <!-- ENDIF @first -->
      <!-- END authentication -->
    </div>
</div>

<div id="row-login-form" class="row hidden">
	<div class="col-md-12">
...
var mustRedirect = !params.get('admin') && params.get('admin') != '';
if (!mustRedirect) {
  document.getElementById('row-btn-portal').style.display = 'none';
  document.getElementById('row-login-form').classList.remove('hidden');
}
